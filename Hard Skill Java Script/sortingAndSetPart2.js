...
function tentukanKelulusan(data) { // parameter isi sesuai kebutuhan
  ...
  // Iterasi data
  for (const pendaftar of data) {
    if (result.zona.A.length < 4) {
      result.zona.A.push(pendaftar.name);
      result.pendaftar_lulus.push(pendaftar);
    } else if (result.zona.B.length < 4) {
      result.zona.B.push(pendaftar.name);
      result.pendaftar_lulus.push(pendaftar);
    } else if (result.zona.C.length < 4) {
      result.zona.C.push(pendaftar.name);
      result.pendaftar_lulus.push(pendaftar);
    } else {
      result.pendaftar_tidak_lulus.push(pendaftar);
    }
  }

  return result;// kembalikan return sesuai expected result
}
